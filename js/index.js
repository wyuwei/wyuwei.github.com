...
var oBtnNext = $('#btn_next');
var aList = $('#ul1 li');
var aClass = [];

$.each(aList, function (index, item) {
    aClass[index] = aList[index].className;
});
...
// 平滑滚动效果
var aNav = $('#nav_list');
var aNavItem = $('#nav_list a');
var root = $('html, body');

$.each(aNavItem, function (index, item) {
    $(this).click(function () {
...
/*添加动画样式*/
function playAnimateDone(cls) {
    var screen = $(cls);
    var animateElement = screenAnimateElements[cls];

    for (var i = 0; i < animateElement.length; i++) {
        var element = $(animateElement[i]);
        var baseCls = element.attr('class');
        element.attr('class', baseCls.replace('_init', '_done'));
    }
}

$(window).bind('scroll', function () {
    var top = $(document).scrollTop();
    if (top > $('.second').offset().top - 50) {
        playAnimateDone('.second');
    }
});
